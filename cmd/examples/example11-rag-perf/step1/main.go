// This example is the baseline for the RAG perf walkthrough: it parses the
// Ultimate Go Notebook chunks, embeds them SEQUENTIALLY (one call per
// chunk), stores them in pgvector, and runs a single end-to-end RAG query
// so we have a working pipeline to compare against in later steps.
//
// # Running the example
//
...
//	$ make kronk-up
//
// First step — sequential embedding only. step2 adds parallel embedding
// with errgroup and reports the speedup over this baseline.

// Example 11 — Step 1 — Sequential Embedding Baseline
package main
...
func run() error {
	...
	if err := insertDocuments(ctx, db, seqDocs); err != nil {
		return err
	}
	...
	// region 4) End-to-end RAG query.

	fmt.Print("\n============================================================\n")
	fmt.Print("4) End-to-End RAG Query\n")
	fmt.Print("============================================================\n")
