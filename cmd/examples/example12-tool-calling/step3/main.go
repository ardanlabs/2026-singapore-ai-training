// This example takes step2 and closes the two-phase tool calling loop:
// after the model signals a `tool_calls` response, the requested
// `tool_search_book` function is actually executed (running a pgvector
// similarity search against the ingested book), the tool result is appended
// to the message history with role "tool", and the model is called a second
// time to produce a natural-language answer grounded in the search result.
//
// # Running the example
//
...
//
// Phase 2 of the two-phase protocol: execute the tool, build the
// assistant/tool reply pair, send the message history back, and print the
// final natural-language answer. The interactive REPL with caching arrives
// in step4.

// Example 12 — Step 3 — Execute Tool + Feed Result Back
package main
...
func run() error {
	...
	// -------------------------------------------------------------------------
	// Step 03
	// 5) Tool calling demo — phase 2 (execute tool + final answer).
	// region Step 03

	fmt.Print("\n============================================================\n")
	fmt.Print("5) Tool Calling Demo — Phase 2 (Execute + Answer)\n")
	fmt.Print("============================================================\n")
	...
	answer, err := chatWithTools(ctx, cln, db, embedClient, messages)
	if err != nil {
		return fmt.Errorf("chat with tools: %w", err)
	}
...
// =============================================================================
// Step 03
// Tool execution and two-phase exchange helper.

// Step 03
func executeSearchBook(ctx context.Context, db *sqlx.DB, embedClient *client.LLM, args map[string]any) map[string]any {
	query, _ := args["query"].(string)
	if query == "" {
		return map[string]any{"status": "ERROR", "error": "missing query argument"}
	}

	results, err := rag.SearchDocuments(ctx, db, embedClient, query, 5)
	if err != nil {
	...
	if len(results) == 0 {
		return toolSuccessResponse(
			"answer", "No passages found in the Go Notebook for this query.",
			"matches", 0,
		)
	}
	...
	for i, r := range results {
		passages[i] = fmt.Sprintf("[similarity=%.3f]\n%s", r.Similarity, r.Text)
	}
...
// Step 03
//
// chatWithTools runs a one-shot, two-phase tool-calling exchange. If the model
// responds with finish_reason "tool_calls", the requested tool is executed and
// the result is sent back for a final natural-language answer. Otherwise the
// direct content is returned.
func chatWithTools(ctx context.Context, cln *client.Client, db *sqlx.DB, embedClient *client.LLM, messages []client.D) (string, error) {
	d := client.D{
		"model":          llmModel,
		"messages":       messages,
		"temperature":    0.1,
	...
	// Phase 1: Model may request a tool call.
	var chat toolChat
	if err := cln.Do(ctx, http.MethodPost, llmURL, d, &chat); err != nil {
		return "", fmt.Errorf("phase 1 chat completions: %w", err)
	}
	...
	// Phase 2: Execute the tool and send the result back.
	var toolResult map[string]any
	switch tc.Function.Name {
	case "tool_search_book":
		toolResult = executeSearchBook(ctx, db, embedClient, tc.Function.Arguments)
	default:
		toolResult = map[string]any{"status": "ERROR", "error": fmt.Sprintf("unknown tool %q", tc.Function.Name)}
	}

	resultJSON, _ := json.Marshal(toolResult)
	fmt.Printf("  Tool result: %s\n\n", resultJSON)

	tcArgsJSON, _ := json.Marshal(tc.Function.Arguments)
	...
	var finalChat toolChat
	if err := cln.Do(ctx, http.MethodPost, llmURL, d, &finalChat); err != nil {
		return "", fmt.Errorf("phase 2 chat completions: %w", err)
	}

	if len(finalChat.Choices) == 0 {
		return "", fmt.Errorf("no final response")
	}

	return finalChat.Choices[0].Message.Content, nil
}

// Step 03
func toolSuccessResponse(keyValues ...any) map[string]any {
