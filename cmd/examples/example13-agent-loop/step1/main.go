// This step writes the agent loop by hand. The lesson: an "agent" is a
// for loop that re-prompts the model until the model stops asking for
// tools.
//
// What changed from example12: example12 ran a single tool call inside
// a one-shot function (chatWithTools); here, the loop iterates until the
// model produces a final answer with no tool_calls, so the model can
// chain multiple tool invocations in a single user turn. A second tool —
// tool_get_reading_progress — is registered so the dispatch is a real
// switch, not a single-case if.
//
// Tool implementations are plain functions returning plain map[string]any
// results: no Tool interface, no {status, data} envelope, no Agent type.
// Those abstractions arrive in step2 and step3.
//
// # Running the example
//
...
// Example 13 — Step 1 — Agent Loop By Hand
package main
...
func run() error {
	...
	// region Tool definitions sent to the model.

	toolDocs := []client.D{
		searchBookToolDef(),
		readingProgressToolDef(),
	}
	...
	for {
		...
		for {
			...
			// No tool calls — model produced its final answer. Record it
			// and break out to the next user prompt.
			...
			// Echo the assistant's tool_calls turn into the conversation
			// so the model sees its own request on the next pass.
			conversation = append(conversation, assistantToolCallsMsg(choice.Message.ToolCalls))

			// Dispatch each tool call with a switch on the tool name.
			for _, tc := range choice.Message.ToolCalls {
				argsJSON, _ := json.Marshal(tc.Function.Arguments)
				fmt.Printf("\u001b[92m%s(%s)\u001b[0m:\n", tc.Function.Name, argsJSON)

				var result map[string]any
				switch tc.Function.Name {
				case "tool_search_book":
					result = doSearchBook(ctx, db, embedClient, tc.Function.Arguments)
				case "tool_get_reading_progress":
					result = doReadingProgress(tc.Function.Arguments)
				default:
					result = map[string]any{"error": fmt.Sprintf("unknown tool %q", tc.Function.Name)}
				}
...
func doSearchBook(ctx context.Context, db *sqlx.DB, embedLLM *client.LLM, args map[string]any) map[string]any {
	query, _ := args["query"].(string)
	if query == "" {
		return map[string]any{"error": "missing query argument"}
	}

	results, err := rag.SearchDocuments(ctx, db, embedLLM, query, 5)
	if err != nil {
...
func readingProgressToolDef() client.D {
	return client.D{
		"type": "function",
		"function": client.D{
...
func doReadingProgress(args map[string]any) map[string]any {
	_ = args
	return map[string]any{
