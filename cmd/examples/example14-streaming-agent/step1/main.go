// Step 1 — Non-streaming baseline. Mirrors example13-agent-loop: the agent
// uses the blocking client.Client and prints the full assistant response only
// after the model finishes generating. The same tool_search_book (pgvector
// similarity search) and tool_get_reading_progress stub are wired in. Later
// steps progressively add SSE streaming, reasoning rendering, a latency
// ticker, and a usage/context-window panel.
//
// # Running the example
//
//	$ make example14-step1
//
...
// Example 14 — Streaming Agent (Step 1)
package main
