// This step introduces the natural-language SQL tool (tool_query_db)
// alongside the two base tools carried over from example14. The agent
// now exposes three real tools:
//
//   - tool_search_book: pgvector similarity search over the ingested book.
//   - tool_get_reading_progress: stub that returns the user's progress.
//   - tool_query_db: receives a natural-language question, uses the LLM to
//     generate a SELECT statement, executes it against the notebook schema
//     (highlights, bookmarks, chapters), and returns result rows as JSON.
//
// The QueryDB type, its prompt, and its registration in NewAgent are all
// new in this step. Defense for the new SQL tool is limited to the
// SELECT-only prompt; nothing here stops a misbehaving model from
// generating INSERT/UPDATE/DELETE/DDL. Step 2 hardens this by opening a
// second read-only pool dedicated to the tools.
//
// # Running the example
//
...
// Example 15 — Step 1 — Add NL→SQL Tool
package main
...
Step 1 — Classify the user message into exactly one of the following. Evaluate the rules IN ORDER and STOP at the first match. Do not reconsider earlier rules once a later one is reached.
  (a) A question about the notebook database — anything mentioning "highlights", "bookmarks", "chapters" (as a list/count, not contents), "how many", "count", "list", "stats", "most recent", or similar database/aggregate phrasing → go to Step 2C. STOP — do not consider any rule below.
  (b) A question about the user's reading progress (where they are, what page, percent complete) → go to Step 2B.
  (c) A question about Go programming or about the contents of the book (any Go topic: interfaces, goroutines, channels, slices, maps, errors, generics, packages, modules, testing, performance, etc.; OR what the book says/covers, author, title) → go to Step 2A.
...
func run() error {
	...
	// -------------------------------------------------------------------------
	// Pick the connection pool the agent's tools will use. In this step that
	// is the same writable pool used for seeding; step 2 swaps it for a
	// read-only pool by reassigning this variable.
	// region Pick the connection pool the agent's tools will use. In this step that
	...
	agent, err := NewAgent(getUserMessage, chatClient, embedClient, agentDB)
	if err != nil {
