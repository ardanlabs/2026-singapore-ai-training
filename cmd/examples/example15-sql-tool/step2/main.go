// This step hardens the NL→SQL tool from step 1 by adding a SECOND
// *sqlx.DB opened with sqldb.Config{ReadOnly: true}. Every connection in
// that pool starts with `default_transaction_read_only=on`, so the LLM
// cannot accidentally (or maliciously) issue INSERT/UPDATE/DELETE/DDL —
// writes fail at the database layer. The SELECT-only prompt is
// defense-in-depth on top.
//
// The only changes over step 1 are in main.go:
//
//   - A new sqldb.Open call for the read-only pool.
//   - One reassignment (`agentDB = readOnlyDB`) that points the agent's
//     tools at that read-only pool instead of the writable pool used for
//     seeding. The QueryDB and SearchBook tools are unchanged; they
//     simply receive a *sqlx.DB whose transactions reject writes.
//
// # Running the example
//
...
//
//	A second, read-only *sqlx.DB pool dedicated to the agent's tools, so
//	any SQL the LLM generates is rejected by Postgres unless it is a
//	SELECT.

// Example 15 — Step 2 — Add Read-Only DB
package main
...
func run() error {
	...
	// -------------------------------------------------------------------------
	// -------------------------------------------------------------------------
	// Open a second, read-only connection pool for the tools. Every
	// connection in this pool starts with default_transaction_read_only=on,
	// so SQL the LLM generates cannot accidentally write to the database.
	// region Open a second, read-only connection pool for the tools. Every
