// This step takes step1 and adds panic recovery to safeCallTool: a
// deferred recover() now turns a panicking tool into a structured FAILED
// envelope instead of crashing the agent. tool_crash (the deliberate
// panicker introduced in step1) still panics; the difference is that the
// REPL now keeps running and the model receives a FAILED tool response
// it can react to. A slow tool can still wedge the loop — step3 layers
// a per-tool timeout on top.
//
// # Running the example
//
...
//
//	A deferred recover() inside safeCallTool that catches tool panics and
//	returns a FAILED envelope so the agent loop keeps running.

// Example 16 — Step 2 — Add Panic Recovery
package main
