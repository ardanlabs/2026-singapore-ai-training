// This example takes the naive baseline from step1 and adds a second run with
// IncrementalCache=true. With IMC enabled, the engine remembers the KV state
// of the longest matching message-array prefix from the previous request on
// the same slot and only re-evaluates the new tail. Requests 2 and 3 of the
// IMC run should show dramatically lower prompt-eval latency than the naive
// run, because the stable system prompt is reused instead of re-prefilled.
//
// # Running the example
//
//	$ make example18-step2
//
...
//
// Second benchmark with IMC enabled and a side-by-side comparison table.
// Makes the prefix-cache win visible in TTFT and prompt_tokens.

// Example 18 — Step 2 — IMC Enabled + Comparison
package main
...
func run() error {
	...
	// -------------------------------------------------------------------------
	// Step 02
	// Prefix-stable approach (IMC enabled).
	// region Step 02

	fmt.Println("\n============================================================")
	fmt.Println("Prefix-Stable Approach — IMC Enabled (IncrementalCache=true)")
	fmt.Println("============================================================")

	cachedResults, err := benchmarkWithConfig(mp, systemPrompt, questions, true)
	if err != nil {
		return fmt.Errorf("cached benchmark: %w", err)
	}
	...
	fmt.Printf("\n  %-10s  %-15s  %-15s  %-15s  %-15s\n", "Request", "Naive TTFT", "Cached TTFT", "Naive Prompt", "Cached Prompt")
	fmt.Printf("  %-10s  %-15s  %-15s  %-15s  %-15s\n", "-------", "----------", "-----------", "------------", "-------------")

	for i := range questions {
		fmt.Printf("  %-10d  %-15s  %-15s  %-15d  %-15d\n",
			i+1,
			fmt.Sprintf("%.0f ms", naiveResults[i].ttftMS),
			fmt.Sprintf("%.0f ms", cachedResults[i].ttftMS),
			naiveResults[i].promptTokens,
			cachedResults[i].promptTokens)
	}

	fmt.Println("\nTTFT (time-to-first-token) is dominated by prompt prefill. With IMC")
	fmt.Println("enabled, requests 2 and 3 should show dramatically lower TTFT: the")
	fmt.Println("engine reuses the KV state of the longest matching message-array")
	fmt.Println("prefix from the prior request on the same slot, so only the new user")
	fmt.Println("turn is prefilled.")
