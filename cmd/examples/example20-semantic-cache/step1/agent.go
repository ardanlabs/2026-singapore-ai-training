...
// Agent represents the chat agent with hardened tool execution and semantic caching.
type Agent struct {
	...
	// Cache statistics tracked by the SearchBook tool.
	cacheHits          int
	cacheMisses        int
...
func NewAgent(getUserMessage func() (string, bool), chatLLM *client.LLM, embedLLM *client.LLM, db *sqlx.DB, readOnlyDB *sqlx.DB) (*Agent, error) {
	...
	agent.toolDocuments = []client.D{
		...
		RegisterSearchBook(agent.tools, embedLLM, db, &agent),
...
func (a *Agent) Run(ctx context.Context) error {
	...
	fmt.Printf("\nChat with GoNotebook AI [%s] (semantic cache enabled)\n", llmModel)
