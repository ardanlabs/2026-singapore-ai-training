// This example builds on the example15-tool-hardening agent by adding a semantic
// cache layer to the RAG search flow. Instead of only caching exact query
// matches, it embeds every tool_search_book query and compares against cached
// embeddings with cosine similarity. If a semantically similar query has been
// seen before (similarity >= 0.80), the cached response is returned immediately
// — skipping the pgvector search and downstream LLM work. The query embedding
// is still computed on every call because it is needed to look up the cache.
//
// The demo runs three hardcoded questions through the agent (two of which are
// semantically similar) and prints cache statistics at the end.
//
// # Running the example
//
//	$ make example20-step1
//
...
// Example 20 — Step 1 — Semantic Cache
package main
