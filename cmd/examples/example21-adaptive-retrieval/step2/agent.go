...
// Agent represents the chat agent with the gate -> cache -> search pipeline.
type Agent struct {
	...
	// Step 02
	// Per-path counters incremented by the SearchBook tool. gated counts
	// queries rejected by the retrieval gate; cacheHits / cacheMisses count
	// the post-gate cache decision.
	gated              int
	cacheHits          int
	cacheMisses        int
	totalHitSimilarity float64
}

// NewAgent creates a new instance of Agent. db is the writable handle used by
// SearchBook for its semantic cache writes; readOnlyDB is the read-only handle
// used by the SQL tool so LLM-generated queries cannot write.
func NewAgent(getUserMessage func() (string, bool), chatLLM *client.LLM, embedLLM *client.LLM, db *sqlx.DB, readOnlyDB *sqlx.DB) (*Agent, error) {
	agent := Agent{
	...
	agent.toolDocuments = []client.D{
		RegisterQueryDB(agent.tools, chatLLM, readOnlyDB),
		RegisterSearchBook(agent.tools, chatLLM, embedLLM, db, &agent),
		RegisterReadingProgress(agent.tools),
	}
...
func (a *Agent) Run(ctx context.Context) error {
	...
	fmt.Printf("\nChat with GoNotebook AI [%s] (gate -> cache -> search pipeline)\n", llmModel)
