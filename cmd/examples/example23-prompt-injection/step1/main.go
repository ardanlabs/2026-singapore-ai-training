// This example demonstrates a direct prompt injection attack against a
// naive RAG pipeline with no defenses. The attacker wraps a malicious
// instruction inside a benign-looking question so that retrieval returns
// relevant (and confidential) documents from pgvector; the model then
// follows the embedded instruction and emits a TOOL_CALL line that
// toolBrowse executes — simulating data exfiltration.
//
// # Running the example
//
//	$ make example23-step1
//
...
// Example 23 — Step 1 — Direct Injection (No Defenses)
package main
