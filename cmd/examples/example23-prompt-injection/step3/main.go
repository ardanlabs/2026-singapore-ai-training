// This example builds on step2 by adding the first defense: role
// separation. The same direct injection from step1 is replayed, but this
// time the system prompt sits in the dedicated system role and the user
// payload sits in the user role — never concatenated into one string.
// Chat-template role boundaries make the embedded "ignore previous
// instructions" much less effective.
//
// # Running the example
//
//	$ make example23-step3
//
...
//
//	Section 3: Defense A — role separation. The chat call uses an explicit
//	system message with a strict policy and the attacker payload in a
//	separate user message; nothing is concatenated.

// Example 23 — Step 3 — Defense A: Role Separation
package main
