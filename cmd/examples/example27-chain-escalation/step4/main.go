// This example builds on step3 by adding a third defense: a call-chain
// audit that logs the full sequence of tool calls per turn. The audit
// does not block anything on its own — its value is visibility, giving
// operators a reviewable trail of what the agent attempted. The escalation
// prompt now runs through four phases: baseline, call budget, supervision,
// and audit.
//
// # Running the example
//
//	$ make example27-step4
//
...
//
//	A fourth phase that runs the same prompt through an agent with
//	AuditMode=true, printing the ordered sequence of tool calls so the
//	escalation chain is visible in the output even when no defense blocks it.

// Example 27 — Step 4 — Defense C: Call-Chain Audit
package main
